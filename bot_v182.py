"""CakeStampBot v2.1.1 Reply-only stamp workflow + Access Control + Crown."""
import os,json,uuid
...
async def deny(update):
 ...
 if update.callback_query:await update.callback_query.answer("Доступ закрыт",show_alert=True)
...
def crown_kb():return kb([["👑 С короной","Без короны"],["↩️ Главное меню"]])
...
def init_stamp(c,source=None):
 ...
 c.user_data["mode"]="stamp";legacy.ensure_stamp_defaults(c);c.user_data.setdefault("add_crown",False)
 if float(c.user_data.get("text_size_mm",12))<10:c.user_data["text_size_mm"]=12.0
def stamp_summary(c):
 d=c.user_data;src="Текст" if d.get("source")=="text" else "Картинка";shape="Круг" if d.get("base_shape","round")=="round" else "Прямоугольник";lines=["🍰 Проверь настройки:",f"Источник: {src}",f"Размер: {d.get('base_size','105')} мм",f"Форма: {shape}"]
 if d.get("source")=="text":lines += [f"Текст: {d.get('text','')}",f"Шрифт: {legacy.stamp_font_name(c)}",f"Высота: {float(d.get('text_size_mm',12)):g} мм",f"Расположение: { {'normal':'Обычный','top':'Сверху','bottom':'Снизу','full':'По окружности'}.get(d.get('text_path'),'Обычный') }"]
 lines += ["Линия: 0.25 мм",f"Сердце: {'Да' if d.get('add_heart') else 'Нет'}",f"Корона сверху: {'Да' if d.get('add_crown') else 'Нет'}",f"3MF: {'Собрать' if d.get('layout_mode')=='assembled' else 'Отдельно'}"];return "\n".join(lines)
async def start_v190(update,context):context.user_data.clear();await update.message.reply_text("CakeStampBot v2.1.1\n\nВыбери действие в меню ниже 👇",reply_markup=legacy.main_menu_keyboard())
async def help_v190(update,context):await update.message.reply_text("Помощь CakeStampBot v2.1.1\n\n🍰 Штамп: Blender Engine + сердце + корона.\n👑 Корона добавляется отдельной деталью сверху.\n🔒 Доступ по Telegram ID.\n🎂 Топпер без изменений.",reply_markup=legacy.main_menu_keyboard())
async def stamp_menu(update,context):context.user_data.clear();context.user_data.update(mode="stamp",step="source");await update.message.reply_text("Режим: 🍰 Штамп. Выбери источник:",reply_markup=source_kb())
async def photo_router(update,context):
 if context.user_data.get("mode")!="stamp" or context.user_data.get("source")!="image":return await update.message.reply_text("Сначала выбери 🍰 Штамп → 🖼 Картинка / логотип.",reply_markup=legacy.main_menu_keyboard())
 f=await update.message.photo[-1].get_file();p=legacy.UPLOAD_DIR/f"{uuid.uuid4().hex[:10]}.jpg";await f.download_to_drive(str(p));context.user_data["image_path"]=str(p);init_stamp(context,"image");context.user_data["step"]="size";await update.message.reply_text("Картинку получил. Выбери размер штампа:",reply_markup=size_kb())
async def text_router(update,context):
 ...
 if step=="text":init_stamp(context,"text");d["text"]=text;d["step"]="size";return await update.message.reply_text("Выбери размер штампа:",reply_markup=size_kb())
 if step=="size":
 ...
 if step=="shape":
  ...
  d["step"]="font" if d.get("source")=="text" else "heart";return await update.message.reply_text("Выбери шрифт:" if d["step"]=="font" else "Добавить сердечко?",reply_markup=font_kb() if d["step"]=="font" else heart_kb())
 if step=="rect":
  ...
  d["base_size"]=vals[text];d["step"]="font" if d.get("source")=="text" else "heart";return await update.message.reply_text("Выбери шрифт:" if d["step"]=="font" else "Добавить сердечко?",reply_markup=font_kb() if d["step"]=="font" else heart_kb())
 if step=="font":
 ...
 if step=="heart":
  ...
  d["step"]="crown";return await update.message.reply_text("Добавить корону сверху?",reply_markup=crown_kb())
 if step=="crown":
  if text=="👑 С короной":d["add_crown"]=True
  elif text=="Без короны":d["add_crown"]=False
  else:return await update.message.reply_text("Добавить корону сверху?",reply_markup=crown_kb())
...
async def callback_guard(update,context):await update.callback_query.answer("Используй нижнее меню",show_alert=False)
async def main_post_init(app):await legacy.post_init(app);await app.bot.set_my_commands([("start","Главное меню"),("stamp","Штамп"),("topper","Топпер"),("queue","Очередь"),("help","Помощь")]);legacy.logger.info("CakeStampBot v2.1.1 Crown workflow started")
def main():
 app=Application.builder().token(legacy.BOT_TOKEN).post_init(main_post_init).post_shutdown(legacy.post_shutdown).build();app.add_handler(CommandHandler("start",guarded(start_v190)));app.add_handler(CommandHandler("help",guarded(help_v190)));app.add_handler(CommandHandler("stamp",guarded(stamp_menu)));app.add_handler(CommandHandler("topper",guarded(legacy.topper_cmd)));app.add_handler(CommandHandler("queue",guarded(legacy.queue_cmd)));app.add_handler(CommandHandler("users",users_cmd));app.add_handler(CommandHandler("adduser",adduser_cmd));app.add_handler(CommandHandler("deluser",deluser_cmd));app.add_handler(CallbackQueryHandler(guarded(callback_guard)));app.add_handler(MessageHandler(filters.PHOTO,guarded(photo_router)));app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,guarded(text_router)));app.add_error_handler(legacy.error_handler);app.run_polling()
