...
def _load_allowed():
    try:
        ...
        legacy.logger.exception("Failed to read access list")
    return set()
...
def is_allowed(user_id):
    uid=int(user_id)
    return uid in ADMIN_IDS or uid in ENV_ALLOWED_IDS or uid in _load_allowed()
...
async def access_guard(handler, update, context):
    uid=update.effective_user.id if update.effective_user else 0
    if not is_allowed(uid): return await deny(update)
    return await handler(update, context)

def guarded(handler):
    async def wrapper(update, context): return await access_guard(handler, update, context)
    return wrapper

async def users_cmd(update, context):
    uid=update.effective_user.id
    if not is_admin(uid): return await deny(update)
    runtime=_load_allowed(); all_users=sorted(ADMIN_IDS | ENV_ALLOWED_IDS | runtime)
    lines=["🔐 Пользователи с доступом:"]
    for x in all_users:
        role=" 👑 admin" if x in ADMIN_IDS else ""
        source=" (env)" if x in ENV_ALLOWED_IDS and x not in ADMIN_IDS else ""
        lines.append(f"• {x}{role}{source}")
...
async def adduser_cmd(update, context):
    ...
    ids=_load_allowed(); ids.add(target); _save_allowed(ids)
    await update.effective_message.reply_text(f"✅ Доступ выдан: {target}")
...
async def deluser_cmd(update, context):
    ...
    if target in ENV_ALLOWED_IDS:
        return await update.effective_message.reply_text(f"⚠️ {target} остаётся разрешён через ALLOWED_USER_IDS в окружении.")
...
def compact_keyboard(c):
    _ensure_defaults(c); d=c.user_data
    path={"normal":"Обычный","top":"Сверху","bottom":"Снизу","full":"По окружности"}.get(d.get("text_path"),"Обычный")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔵 Диаметр · {d.get('base_size','105')} мм",callback_data="qcat:size"),InlineKeyboardButton(f"🔤 Шрифт · {legacy.stamp_font_name(c)}",callback_data="qcat:font")],
        [InlineKeyboardButton(f"↕️ Размер · {float(d.get('text_size_mm',12)):g} мм",callback_data="qcat:h"),InlineKeyboardButton(f"〰️ {path}",callback_data="qcat:path")],
        [InlineKeyboardButton(f"❤️ Сердце · {'Да' if d.get('add_heart') else 'Нет'}",callback_data="qs:heart"),InlineKeyboardButton(f"🧩 3MF · {'Собрать' if d.get('layout_mode')=='assembled' else 'Отдельно'}",callback_data="qs:layout")],
        [InlineKeyboardButton("🚀 СОЗДАТЬ 3MF",callback_data="create")],[InlineKeyboardButton("↩️ Назад в меню",callback_data="restart")]])
...
def category_keyboard(c,kind):
    ...
    if kind=="size":
        ...
        return InlineKeyboardMarkup([[InlineKeyboardButton(("✓ " if cur==v else "")+v+" мм",callback_data=f"qs:size:{v}") for v in vals[:3]],[InlineKeyboardButton(("✓ " if cur==v else "")+v+" мм",callback_data=f"qs:size:{v}") for v in vals[3:]],back])
    if kind=="h":
        cur=float(d.get("text_size_mm",12)); vals=[10,11,12,13,14,15,16]
        return InlineKeyboardMarkup([[InlineKeyboardButton(("✓ " if cur==v else "")+f"{v} мм",callback_data=f"qs:h:{v}") for v in vals[:4]],[InlineKeyboardButton(("✓ " if cur==v else "")+f"{v} мм",callback_data=f"qs:h:{v}") for v in vals[4:]],back])
    if kind=="font":
    ...
    return InlineKeyboardMarkup([[InlineKeyboardButton(("✓ " if cur==v else "")+name,callback_data=f"qs:path:{v}") for v,name in vals[:2]],[InlineKeyboardButton(("✓ " if cur==v else "")+name,callback_data=f"qs:path:{v}") for v,name in vals[2:]],back])
...
async def help_v188(update,context):
    await update.message.reply_text("Помощь CakeStampBot v1.8.8\n\n🍰 Штамп: высота 10–16 мм, отступ около 20 мм, линия 0.25 мм.\n🔒 Доступ только для разрешённых Telegram ID.\n🎂 Топпер: без изменений.",reply_markup=legacy.main_menu_keyboard())

async def main_post_init(app):
    await legacy.post_init(app)
    await app.bot.set_my_commands([("start","Главное меню"),("stamp","Штамп"),("topper","Топпер"),("queue","Очередь"),("help","Помощь")])
    if not ADMIN_IDS:
        legacy.logger.warning("ACCESS CONTROL: ADMIN_USER_IDS is empty. No one can administer access until it is set.")
    legacy.logger.info("CakeStampBot v1.8.8 Access Control started; admins=%s", sorted(ADMIN_IDS))

def main():
    app=Application.builder().token(legacy.BOT_TOKEN).post_init(main_post_init).post_shutdown(legacy.post_shutdown).build()
    app.add_handler(CommandHandler("start",guarded(start_v188)))
    app.add_handler(CommandHandler("help",guarded(help_v188)))
    app.add_handler(CommandHandler("stamp",guarded(legacy.stamp_cmd)))
    app.add_handler(CommandHandler("topper",guarded(legacy.topper_cmd)))
    app.add_handler(CommandHandler("queue",guarded(legacy.queue_cmd)))
    app.add_handler(CommandHandler("users",users_cmd))
    app.add_handler(CommandHandler("adduser",adduser_cmd))
    app.add_handler(CommandHandler("deluser",deluser_cmd))
    app.add_handler(CallbackQueryHandler(guarded(compact_callback)))
    app.add_handler(MessageHandler(filters.PHOTO,guarded(legacy.on_photo)))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,guarded(legacy.on_text)))
    app.add_error_handler(legacy.error_handler)
    app.run_polling()
