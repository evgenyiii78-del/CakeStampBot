"""CakeStampBot v1.9.1 stamp text geometry stability.

Text is scaled on the unbuffered centerline, cleaned after scaling, then stroked
at exactly 0.25 mm. Polygon output is repaired conservatively before extrusion.
Topper is intentionally untouched.
"""
...
def _scale_centerline(line_geom, target_block_height_mm: float, max_width_mm: float, max_height_mm: float):
    ...
    factor=min(float(target_block_height_mm)/h, float(max_width_mm)/w, float(max_height_mm)/h)
    scaled=affinity.scale(line_geom,xfact=factor,yfact=factor,origin=(0.0,0.0))
    bx0,by0,bx1,by1=scaled.bounds
    scaled=affinity.translate(scaled,xoff=-(bx0+bx1)/2.0,yoff=-(by0+by1)/2.0)
...
def _polygon_parts(shape):
    if shape is None or shape.is_empty:
        return []
    if isinstance(shape,Polygon): return [shape]
    if isinstance(shape,MultiPolygon): return list(shape.geoms)
...
def build_stamp_from_text(*, text, output_dir, base_size="105", base_shape="round",
    ...
    if base_shape=="rect":
        safe_w=max(10.0,float(rw)-2.0*SAFE_MARGIN_MM); safe_h=max(10.0,float(rh)-2.0*SAFE_MARGIN_MM)
    else:
        safe_w=max(10.0,float(nominal)-2.0*SAFE_MARGIN_MM); safe_h=safe_w
    ...
    def sized_exact_stroke(centerline,_line_width):
        ...
        # Buffer each clean line independently. This avoids noding/LocateFailure
        # on complex MultiLineString intersections produced by skeletonization.
        buffered=[]
        for ln in _line_parts(centerline):
            try:
                poly=ln.buffer(exact_width/2.0,cap_style=1,join_style=1,resolution=16)
        ...
        try:
            ...
            # Keep separate valid polygons if GEOS cannot node the whole set.
            polys=[]
    ...
    try:
        return _se.build_stamp_from_text(
            text=text,output_dir=output_dir,base_size=base_size,base_shape=base_shape,
            line_width=exact_width,font_choice=font_choice,text_path=text_path,
            add_heart=add_heart,layout_mode=layout_mode,
        )
    finally:
