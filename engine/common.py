...
def mask_to_centerline_shape(mask, px_per_mm=24, line_width=.45, smooth=.30):
    ...
    for path in lines:
        ...
        geoms.append(line.buffer(line_width/2,cap_style=1,join_style=1,resolution=32))
    if not geoms: return None
    merged=unary_union(geoms).buffer(0); eps=max(.018,line_width*.10)
    return merged.buffer(eps,resolution=32).buffer(-eps,resolution=32).buffer(0).simplify(.015,preserve_topology=True).buffer(0)
...
def heart_mesh(line_width,height,y=-31):
    ...
    m=extrude_shape(LineString(np.c_[x,yy]).buffer(line_width/2,cap_style=1,join_style=1,resolution=32),height,'Heart'); m.apply_translation([0,y,0]); return m

def preview(path,title,mode,mask=None,note=''):
    img=Image.new('RGB',(1000,1000),(246,243,235)); d=ImageDraw.Draw(img)
    if mode=='stamp': d.ellipse((70,70,930,930),fill=(228,192,120),outline=(130,95,45),width=6)
    else:
        d.rounded_rectangle((110,225,890,690),radius=30,fill=(228,192,120),outline=(130,95,45),width=5)
        d.rounded_rectangle((370,680,415,925),radius=15,fill=(228,192,120),outline=(130,95,45),width=4)
        d.rounded_rectangle((585,680,630,925),radius=15,fill=(228,192,120),outline=(130,95,45),width=4)
    if mask is not None:
