...
def stamp_text_path_keyboard(): return InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Обычный",callback_data="stamp_text_path:normal")],[InlineKeyboardButton("⌒ Сверху",callback_data="stamp_text_path:top"),InlineKeyboardButton("⌣ Снизу",callback_data="stamp_text_path:bottom")],[InlineKeyboardButton("⭕ По всей окружности",callback_data="stamp_text_path:full")]])
...
def log_update(update,event):
    ...
    logger.info("%s | user_id=%s username=%s chat_id=%s",event,getattr(user,"id",None),getattr(user,"username",None),getattr(chat,"id",None))
def ensure_topper_defaults(context):
    context.user_data.setdefault("mode","topper"); context.user_data.setdefault("source","text"); context.user_data.setdefault("font_choice","classic"); context.user_data.setdefault("topper_width",120.0); context.user_data.setdefault("topper_text_height",3.0); context.user_data.setdefault("topper_backing_height",1.2); context.user_data.setdefault("topper_legs","auto")
def stamp_font_name(context):
    c=str(context.user_data.get("font_choice","classic")).strip().lower(); return {"classic":"Classic","comic":"Comic Sans","gost":"GOST"}.get(c,c or "Classic")
def stamp_summary_text(context):
    ...
    return "Проверь настройки штампа:\n\n"+f"• источник: {'текст' if context.user_data.get('source')=='text' else 'картинка'}\n"+f"• размер: {str(context.user_data.get('base_size','105')).replace('x','×')} мм\n"+f"• форма: {'круглая' if context.user_data.get('base_shape')=='round' else 'прямоугольная'}\n"+f"• шрифт: {stamp_font_name(context)}\n• линия: {context.user_data.get('line_width',0.25)} мм\n• текст: {path}\n"+f"• сердце: {'да' if context.user_data.get('add_heart') else 'нет'}\n"+f"• раскладка: {'собрать' if context.user_data.get('layout_mode')=='assembled' else 'отдельно'}"
def topper_summary_text(context):
    ensure_topper_defaults(context); legs=context.user_data.get("topper_legs","auto"); lr={"auto":"авто","one":"1 ножка","two":"2 ножки"}.get(legs,str(legs))
    return f"Проверь настройки топпера:\n\n• ширина: {context.user_data.get('topper_width',120)} мм\n• высота текста: {context.user_data.get('topper_text_height',3.0)} мм\n• подложка под буквами: {context.user_data.get('topper_backing_height',1.2)} мм\n• ножки: {lr}\n\nТоппер будет цельным: текст + подложка под буквами + две отдельные ножки."
async def send_or_edit_summary(query,context): await query.edit_message_text(stamp_summary_text(context) if context.user_data.get("mode")=="stamp" else topper_summary_text(context),reply_markup=create_keyboard())

async def start(update,context):
    log_update(update,"COMMAND /start"); context.user_data.clear(); await update.message.reply_text("CakeStampBot v1.7.1",reply_markup=main_menu_keyboard()); await update.message.reply_text("Выбери режим:",reply_markup=mode_inline_keyboard())
async def help_cmd(update,context):
    log_update(update,"COMMAND /help"); await update.message.reply_text("Помощь CakeStampBot v1.7.1\n\n🍰 Штамп: текст или картинка → PNG + 3MF; имя 3MF берётся из текста.\n🎂 Топпер: текст → единая модель с подложкой под буквами и ножками.\n\nВырубка удалена из проекта.",reply_markup=main_menu_keyboard())
...
async def stamp_cmd(update,context): context.user_data.clear(); context.user_data["mode"]="stamp"; await update.message.reply_text("Режим: 🍰 Штамп. Выбери источник:",reply_markup=source_keyboard("stamp"))
async def topper_cmd(update,context): context.user_data.clear(); context.user_data.update(mode="topper",source="text"); await update.message.reply_text("Режим: 🎂 Топпер. Напиши текст для топпера.")
async def on_text(update,context):
    ...
    context.user_data["text"]=text; await update.message.reply_text("Выбери шрифт:",reply_markup=font_keyboard())
...
async def on_callback(update,context):
    ...
    try:
        ...
        if data.startswith("source:"): context.user_data["source"]=data.split(":",1)[1]; return await q.edit_message_text("Напиши текст." if context.user_data["source"]=="text" else "Пришли картинку или логотип.")
        ...
        if data.startswith("rect_size:"): context.user_data["base_size"]=data.split(":",1)[1]; context.user_data["line_width"]=0.25; context.user_data["text_path"]="normal"; return await q.edit_message_text("Добавить сердечко?",reply_markup=heart_keyboard())
        if data.startswith("stamp_text_path:"): context.user_data["text_path"]=data.split(":",1)[1]; return await q.edit_message_text("Добавить сердечко?",reply_markup=heart_keyboard())
...
def build_model(params):
    ...
    return build_stamp_from_text(text=params["text"],output_dir=str(out),base_size=params.get("base_size","105"),base_shape=params.get("base_shape","round"),line_width=0.25,font_choice=params.get("font_choice","classic"),text_path=params.get("text_path","normal"),add_heart=bool(params.get("add_heart",False)),layout_mode=params.get("layout_mode","assembled"))
async def cake_worker(app):
    q=app.bot_data["cake_queue"]
    while True:
        job=await q.get()
        try:
            await app.bot.send_message(chat_id=job.chat_id,text="🔧 Начал обработку модели...\nВекторный топпер может занять 30–120 секунд."); result=await asyncio.wait_for(asyncio.to_thread(build_model,job.params),timeout=210)
