...
def font_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("Classic SL", callback_data="font:classic"),
                InlineKeyboardButton("Comic SL", callback_data="font:comic"),
            ],
            [InlineKeyboardButton("GOST SL", callback_data="font:gost")],
        ]
    )
...
def topper_legs_keyboard() -> InlineKeyboardMarkup:
    # В v1.5.0 поддерживаются также старые callback-префиксы legs:*
    return InlineKeyboardMarkup(
        [
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # v1.5.0: поддерживаем topper_legs:*, topper_leg:* и старый legs:*.
...
async def enqueue_job(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # v1.5.0 hotfix: do not enqueue a topper job without saved text.
...
def main() -> None:
    ...
    logger.info("CakeStampBot v1.5.0 started")
    print("CakeStampBot v1.5.0 started")
    app.run_polling()
