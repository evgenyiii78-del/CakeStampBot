...
def topper_legs_keyboard() -> InlineKeyboardMarkup:
    # В v1.2.4 поддерживаются также старые callback-префиксы legs:*
    return InlineKeyboardMarkup(
        [
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # v1.2.4: поддерживаем topper_legs:*, topper_leg:* и старый legs:*.
...
async def enqueue_job(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # v1.2.4 hotfix: do not enqueue a topper job without saved text.
...
def main() -> None:
    ...
    logger.info("CakeStampBot v1.2.4 started")
    print("CakeStampBot v1.2.4 started")
    app.run_polling()
