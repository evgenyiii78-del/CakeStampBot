...
def topper_legs_keyboard() -> InlineKeyboardMarkup:
    # В v1.6.6 поддерживаются также старые callback-префиксы legs:*
    return InlineKeyboardMarkup(
        [
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # v1.6.6: поддерживаем topper_legs:*, topper_leg:* и старый legs:*.
...
async def enqueue_job(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # v1.6.6 hotfix: do not enqueue a topper job without saved text.
