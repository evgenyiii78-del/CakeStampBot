...
def topper_legs_keyboard() -> InlineKeyboardMarkup:
    # В v1.2.6 поддерживаются также старые callback-префиксы legs:*
    return InlineKeyboardMarkup(
        [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "🍰 Штамп — centerline-штамп с реальной толщиной линии для крема.\n"
        "🎂 Топпер — TextBase по буквам, две перемычки между строками, ножка отдельно, низ со срезом.",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # v1.2.6: поддерживаем topper_legs:*, topper_leg:* и старый legs:*.
...
async def enqueue_job(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # v1.2.6 hotfix: do not enqueue a topper job without saved text.
...
def main() -> None:
    ...
    logger.info("CakeStampBot v1.2.6 started")
    print("CakeStampBot v1.2.6 started")
    app.run_polling()
