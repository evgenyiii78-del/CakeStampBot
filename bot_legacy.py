...
logger=logging.getLogger("CakeStampBot"); BOT_TOKEN=os.getenv("BOT_TOKEN")
...
def source_keyboard(mode): return InlineKeyboardMarkup([[InlineKeyboardButton("✍️ Текст",callback_data="source:text")]]) if mode=="topper" else InlineKeyboardMarkup([[InlineKeyboardButton("✍️ Текст",callback_data="source:text")],[InlineKeyboardButton("🖼 Картинка / логотип",callback_data="source:image")]])
...
def ensure_stamp_defaults(c):
 d=c.user_data; d.setdefault("mode","stamp"); d.setdefault("source","text"); d.setdefault("base_shape","round"); d.setdefault("base_size","105"); d.setdefault("stamp_size","105"); d.setdefault("font_choice","classic"); d.setdefault("text_size_mm",7.0); d.setdefault("text_path","normal"); d.setdefault("line_width",.25); d.setdefault("add_heart",False); d.setdefault("add_crown",False); d.setdefault("layout_mode","separate")
...
def stamp_quick_keyboard(c):
 ...
 return InlineKeyboardMarkup([[InlineKeyboardButton(mark("60",size=="60"),callback_data="qs:size:60"),InlineKeyboardButton(mark("80",size=="80"),callback_data="qs:size:80"),InlineKeyboardButton(mark("105",size=="105"),callback_data="qs:size:105")],[InlineKeyboardButton(mark("130",size=="130"),callback_data="qs:size:130"),InlineKeyboardButton(mark("145",size=="145"),callback_data="qs:size:145")],[InlineKeyboardButton(mark("Обычный",path=="normal"),callback_data="qs:path:normal"),InlineKeyboardButton(mark("Сверху",path=="top"),callback_data="qs:path:top")],[InlineKeyboardButton(mark("Снизу",path=="bottom"),callback_data="qs:path:bottom"),InlineKeyboardButton(mark("По окружности",path=="full"),callback_data="qs:path:full")],[InlineKeyboardButton(mark("5 мм",h==5),callback_data="qs:h:5"),InlineKeyboardButton(mark("6 мм",h==6),callback_data="qs:h:6"),InlineKeyboardButton(mark("7 мм",h==7),callback_data="qs:h:7")],[InlineKeyboardButton(mark("8 мм",h==8),callback_data="qs:h:8"),InlineKeyboardButton(mark("9 мм",h==9),callback_data="qs:h:9"),InlineKeyboardButton(mark("10 мм",h==10),callback_data="qs:h:10")],[InlineKeyboardButton(mark("Classic",fs=="classic"),callback_data="qs:font:classic"),InlineKeyboardButton(mark("Comic",fs=="comic"),callback_data="qs:font:comic"),InlineKeyboardButton(mark("GOST",fs=="gost"),callback_data="qs:font:gost")],[InlineKeyboardButton(mark("❤️ Сердце",d["add_heart"]),callback_data="qs:heart"),InlineKeyboardButton(mark("👑 Корона",d["add_crown"]),callback_data="qs:crown")],[InlineKeyboardButton(mark("🧩 Отдельно",d["layout_mode"]=="separate"),callback_data="qs:layout")],[InlineKeyboardButton("🚀 СОЗДАТЬ 3MF",callback_data="create")],[InlineKeyboardButton("↩️ Назад в меню",callback_data="restart")]])
...
def topper_summary_text(c):
 ensure_topper_defaults(c); lr={"auto":"авто","one":"1 ножка","two":"2 ножки"}.get(c.user_data.get("topper_legs","auto"),"авто"); return f"Проверь настройки топпера:\n\n• ширина: {c.user_data.get('topper_width',120)} мм\n• высота текста: {c.user_data.get('topper_text_height',3.0)} мм\n• подложка под буквами: {c.user_data.get('topper_backing_height',1.2)} мм\n• ножки: {lr}"
async def start(u,c): c.user_data.clear(); await u.message.reply_text("CakeStampBot v2.1.1",reply_markup=main_menu_keyboard()); await u.message.reply_text("Выбери режим:",reply_markup=mode_inline_keyboard())
async def help_cmd(u,c): await u.message.reply_text("Помощь CakeStampBot v2.1.1\n\n🍰 Штамп: Blender Engine, сердце и корона.\n🎂 Топпер без изменений.",reply_markup=main_menu_keyboard())
async def queue_cmd(u,c): q=c.application.bot_data.get("cake_queue"); await u.message.reply_text(f"В очереди задач: {q.qsize() if q else 0}",reply_markup=main_menu_keyboard())
...
async def on_callback(u,c):
 ...
 try:
  ...
  if data.startswith("qs:"):
   ...
   elif p[1]=="crown": c.user_data["add_crown"]=not bool(c.user_data.get("add_crown"))
  ...
  if data.startswith("stamp_shape:"):
   ...
   c.user_data["base_size"]=c.user_data.get("stamp_size","105"); ensure_stamp_defaults(c)
   if c.user_data.get("source")=="text": return await show_stamp_settings(q,c)
  ...
  if data.startswith("rect_size:"): c.user_data["base_size"]=data.split(":",1)[1]; ensure_stamp_defaults(c); return await q.edit_message_text("Добавить сердечко?",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❤️ С сердцем",callback_data="heart:yes")],[InlineKeyboardButton("Без сердца",callback_data="heart:no")]]))
  ...
  if data.startswith("topper_width:"): ensure_topper_defaults(c); c.user_data["topper_width"]=float(data.split(":",1)[1]); return await q.edit_message_text("Выбери высоту топпера:",reply_markup=topper_text_height_keyboard())
 ...
 except Exception: logger.exception("Callback failed %s",data); await q.message.reply_text("Ошибка обработки кнопки. Попробуй /start.")
...
def build_model(p):
 ...
 if p.get("mode")=="topper": return build_topper_from_text(text=p["text"],output_dir=str(out),width_mm=float(p.get("topper_width",120)),font_choice=p.get("font_choice","classic"),text_height=float(p.get("topper_text_height",3)),backing_height=float(p.get("topper_backing_height",1.2)),legs=p.get("topper_legs","auto"))
 if p.get("source")=="image": return build_stamp_from_image(image_path=p["image_path"],output_dir=str(out),base_size=p.get("base_size","105"),base_shape=p.get("base_shape","round"),line_width=.25,add_heart=bool(p.get("add_heart",False)),layout_mode=p.get("layout_mode","assembled"))
 return build_stamp_from_text(text=p["text"],output_dir=str(out),base_size=p.get("base_size","105"),base_shape=p.get("base_shape","round"),line_width=.25,font_choice=p.get("font_choice","classic"),text_path=p.get("text_path","normal"),text_size_mm=float(p.get("text_size_mm",12)),add_heart=bool(p.get("add_heart",False)),add_crown=bool(p.get("add_crown",False)),layout_mode=p.get("layout_mode","separate"))
